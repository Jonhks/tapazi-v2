...
const History = () => {
  ...
  const { data: tournaments, isLoading: loadingTournaments } = useQuery({
    queryKey: ["nflHistoryTournaments", userId, sportId],
    queryFn: () => getTournaments(sportId),
    enabled: Boolean(sportId),
  });

  const { data: teamsPerYearLog } = useQuery({
    queryKey: ["teamsPerYearLog", userId, selectedTournament?.id],
  ...
  if (loadingTournaments || loadingPerfectPortfolios || loadingHistoryAllRounds)
    return <Loader />;
