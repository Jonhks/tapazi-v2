  ...
  return (
    <Box sx={{ p: 3 }}>
      {portfolio?.teams?.map((team, teamIndex: number) => (
        <div key={teamIndex} className={classes.containerDropdown}>
          <BallIcon />
          <Dropdown
            disabled={isReadOnly}
            ...
            handleChange={(selectedTeam) => handleTeamChange(selectedTeam, teamIndex)}
          />
        </div>
      ))}

      <Grid container display="flex" justifyContent="end" sx={{ mt: 2 }}>
      ...
      <Grid container m={2} justifyContent="end" spacing={2}>
        {isReadOnly ? (
          isValidTournament && (
